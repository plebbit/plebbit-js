...
export class SubplebbitClientsManager extends PlebbitClientsManager {
    ...
    async fetchNewUpdateForSubplebbit(subAddress) {
        return this._withInflightSubplebbitFetch(subAddress, async () => {
            ...
            let subRes;
            const areWeConnectedToKuboOrHelia = Object.keys(this._plebbit.clients.kuboRpcClients).length > 0 ||
            ...
            const throwIfGatewayRespondsWithInvalidSubplebbit = async (gatewayRes) => {
                ...
                if (errorWithinRecord) {
                    ...
                    if (errorWithinRecord.code === "ERR_SUBPLEBBIT_SIGNATURE_IS_INVALID") {
                        ...
                        log.error(`Gateway ${gatewayUrl} returned subplebbit record with invalid signature. ` +
