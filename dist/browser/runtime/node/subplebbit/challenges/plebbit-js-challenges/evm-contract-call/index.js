...
const verifyAuthorWalletAddress = async (props) => {
    ...
    if (isStringDomain(authorWallet.address)) {
        ...
        if (resolvedWalletAddress !== publicationSignatureAddress) {
            const failedMsg = "The author wallet address's plebbit-author-address text record should resolve to the public key of the signature";
            log.error(`The author wallet address (${authorWallet.address}) resolves to an incorrect value (${resolvedWalletAddress}), but it should resolve to ${publicationSignatureAddress}`);
            return failedMsg;
        }
    }
...
const verifyAuthorNftWalletAddress = async (props) => {
    ...
    const viemClient = props.plebbit._domainResolver._createViemClientIfNeeded(nftAvatar.chainTicker, chainProvider.urls[0]);
    let currentOwner;
    try {
...
const getContractCallResponse = async (props) => {
    ...
    try {
        const viemClient = props.plebbit._domainResolver._createViemClientIfNeeded(props.chainTicker, _getChainProviderWithSafety(props.plebbit, props.chainTicker).urls[0]);
...
const evaluateConditionString = (condition, responseValue) => {
    ...
    if (typeof conditionValueParsed !== typeof responseValueParsed)
        throw Error("value of condition and response should be the same");
    const result = operatorInCondition === "="
        ...
        : operatorInCondition === ">"
            ...
            : operatorInCondition === "<"
                ...
                : undefined;
    if (result === undefined)
        throw Error("Failed to parse condition. Please double check code and set condition");
    return result;
};
...
const getChallenge = async (subplebbitChallengeSettings, challengeRequestMessage, challengeIndex, subplebbit) => {
    ...
    if (!nftWalletAddressFailureReason)
        return { success: true };
    const errorString = `Author (${publication.author.address}) has failed all EVM challenges, ` +
