...
import { ChallengeAnswerMessageSignedPropertyNames, ChallengeMessageSignedPropertyNames, ChallengeRequestMessageSignedPropertyNames, ChallengeVerificationMessageSignedPropertyNames, CommentEditSignedPropertyNames, CommentSignedPropertyNames, CommentUpdateSignedPropertyNames, SubplebbitSignedPropertyNames, VoteSignedPropertyNames } from "./constants.js";
...
async function _validateAuthorIpns(author, signer, plebbit) {
...
async function _signJson(signedPropertyNames, publication, signer, log) {
...
export async function signComment(comment, signer, plebbit) {
...
export async function signVote(vote, signer, plebbit) {
...
export async function signCommentEdit(edit, signer, plebbit) {
...
const bufferCleanedObject = (signedPropertyNames, objectToSign) => {
...
const _verifyJsonSignature = async (publicationToBeVerified) => {
    ...
    const signatureIsValid = await verifyBufferEd25519(cborg.encode(propsToSign), uint8ArrayFromString(publicationToBeVerified.signature.signature, "base64"), publicationToBeVerified.signature.publicKey);
...
export async function verifyComment(comment, resolveAuthorAddresses, clientsManager, overrideAuthorAddressIfInvalid) {
...
export async function verifySubplebbit(subplebbit, resolveAuthorAddresses, clientsManager, overrideAuthorAddressIfInvalid) {
    ...
    if (subplebbit.posts?.pages)
        for (const pageSortName of Object.keys(subplebbit.posts.pages)) {
            const pageValidity = await verifyPage(subplebbit.posts.pageCids[pageSortName], subplebbit.posts.pages[pageSortName], resolveAuthorAddresses, clientsManager, subplebbit.address, undefined, overrideAuthorAddressIfInvalid);
...
export async function verifyCommentUpdate(update, resolveAuthorAddresses, clientsManager, subplebbitAddress, comment, overrideAuthorAddressIfInvalid) {
    ...
    const cacheKey = sha256(update.signature.signature + resolveAuthorAddresses + subplebbitAddress + JSON.stringify(comment) + overrideAuthorAddressIfInvalid);
...
export async function verifyPage(pageCid, page, resolveAuthorAddresses, clientsManager, subplebbitAddress, parentCommentCid, overrideAuthorAddressIfInvalid) {
    ...
    for (const pageComment of page.comments) {
        ...
        const commentSignatureValidity = await verifyComment(pageComment.comment, resolveAuthorAddresses, clientsManager, overrideAuthorAddressIfInvalid);
        if (!commentSignatureValidity.valid)
            return commentSignatureValidity;
        if (commentSignatureValidity.derivedAddress)
            shouldCache = false;
        const commentUpdateSignatureValidity = await verifyCommentUpdate(pageComment.update, resolveAuthorAddresses, clientsManager, subplebbitAddress, pageComment.comment, overrideAuthorAddressIfInvalid);
