...
exports.default = exports.SIGNER_USAGES = void 0;
...
const debug = (0, _debug.default)("plebbit-js:DbHandler");
const SIGNER_USAGES = {
  SUBPLEBBIT: "subplebbit",
  COMMENT: "comment"
};
exports.SIGNER_USAGES = SIGNER_USAGES;
...
class DbHandler {
  constructor(dbConfig, subplebbit) {
    ...
    this.knex = (0, _knex.default)(dbConfig);
  ...
  async upsertComment(postOrComment, challengeRequestId, trx = undefined) {
    return new Promise(async (resolve, reject) => {
      ...
      const dbObject = originalComment ? { ...(0, _Util.removeKeysWithUndefinedValues)(originalComment.toJSONForDb(challengeRequestId)),
        ...(0, _Util.removeKeysWithUndefinedValues)(postOrComment.toJSONForDb(challengeRequestId))
  ...
  async querySubplebbitMetrics(trx) {
    return new Promise(async (resolve, reject) => {
      ...
      for (const metricType of ["ActiveUserCount", "PostCount"]) for (const timeframe of Object.keys(_Util.TIMEFRAMES_TO_SECONDS)) {
...
async function _createChallengesTable2() {
  await this.knex.schema.createTable(TABLES.CHALLENGES, table => {
    ...
    table.enum("type", Object.values(_Challenge.PUBSUB_MESSAGE_TYPES)).notNullable();
...
async function _createCommentsFromRows2(commentsRows, trx) {
  return new Promise(async (resolve, reject) => {
    if (!commentsRows) resolve([undefined]);else {
      ...
      commentsRows = commentsRows.map(props => (0, _Util.replaceXWithY)(props, null, undefined)); // Replace null with undefined to save storage (undefined is not included in JSON.stringify)

      const authors = (await _classPrivateMethodGet(this, _baseTransaction, _baseTransaction2).call(this, trx)(TABLES.AUTHORS).whereIn("address", commentsRows.map(post => post.authorAddress))).map(authorProps => new _Author.default(authorProps));
      const comments = commentsRows.map(commentProps => {
        const props = { ...commentProps,
          "author": authors.filter(author => author.address === commentProps.authorAddress)[0]
        };
        if (props["title"]) return new _Post.default(props, this.subplebbit);else return new _Comment.Comment(props, this.subplebbit);
      });
      resolve(comments);
    }
  });
...
async function _createVotesFromRows2(voteRows, trx) {
  return new Promise(async (resolve, reject) => {
    if (!voteRows) resolve([undefined]);else {
      ...
      voteRows = voteRows.map(props => (0, _Util.replaceXWithY)(props, null, undefined)); // Replace null with undefined to save storage (undefined is not included in JSON.stringify)

      const authors = (await _classPrivateMethodGet(this, _baseTransaction, _baseTransaction2).call(this, trx)(TABLES.AUTHORS).whereIn("address", voteRows.map(vote => vote.authorAddress))).map(authorProps => new _Author.default(authorProps));
      const votes = voteRows.map(voteProps => {
        ...
        return new _Vote.default(props, this.subplebbit);
      });
      resolve(votes);
    }
  });
...
async function _querySubplebbitActiveUserCount2(timeframe, trx) {
  return new Promise(async (resolve, reject) => {
    let from = (0, _Util.timestamp)() - _Util.TIMEFRAMES_TO_SECONDS[timeframe];

    if (from === Number.NEGATIVE_INFINITY) from = 0;
    const to = (0, _Util.timestamp)(); // TODO this could be done in a single query
