...
export class BaseClientsManager {
    ...
    async _verifyContentIsSameAsCid(content, cid) {
        const calculatedCid = await calculateIpfsHash(content);
    ...
    async _setHttpRouterOptionsOnIpfsNode(ipfsClient, routingValue) {
    ...
    async retrySettingHttpRoutersOnIpfsNodes() {
        if (!this._plebbit.httpRoutersOptions)
            throw Error("Need http router options to be defined");
