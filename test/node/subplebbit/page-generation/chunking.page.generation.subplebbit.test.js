...
function resetCommentFactory() {
    commentCounter = 0;
}

function createCommentEntry(contentLengthOrOptions = 0) {
...
function normalizeCommentOptions(contentLengthOrOptions) {
...
function buildContentString(label, targetBytes) {
...
function createReplyEntries({ parentCid, parentDepth, postCid, subplebbitAddress, count, contentLength }) {
...
function createShallowReplyEntry({ parentCid, parentDepth, postCid, subplebbitAddress, label, contentLength }) {
...
function measureCommentSize(entry) {
    return Buffer.byteLength(JSON.stringify(entry), "utf8");
}

function measurePageSize(comments, { includeNextCid } = { includeNextCid: false }) {
