...
describeSkipIfRpc('subplebbit.features.pseudonymityMode="per-post"', () => {
    describe.concurrent("local anonymization", () => {
        ...
        it('Spec: sub re-signs comments with an anonymized author address when pseudonymityMode="per-post"', async () => {
            ...
            const aliasRow = context.subplebbit._dbHandler.queryPseudonymityAliasByCommentCid(post.cid);
        ...
        it("Spec: same signer maps to the same anonymized author address within a single post thread", async () => {
            ...
            const aliasRow = context.subplebbit._dbHandler.queryPseudonymityAliasForPost(threadSigner.publicKey, post.cid);
            expect(aliasRow).to.exist;
            const aliasSigner = await context.publisherPlebbit.createSigner({ privateKey: aliasRow.aliasPrivateKey, type: "ed25519" });
        ...
        it("Spec: two different signers never share the same anonymized author address within a single post thread", async () => {
            ...
            const firstAlias = context.subplebbit._dbHandler.queryPseudonymityAliasForPost(authorSigner.publicKey, post.cid);
            const secondAlias = context.subplebbit._dbHandler.queryPseudonymityAliasForPost(otherSigner.publicKey, post.cid);
        ...
        it("Spec: author.address domains resolve and are anonymized per post thread", async () => {
            ...
            const aliasRow = context.subplebbit._dbHandler.queryPseudonymityAliasForPost(domainSigner.publicKey, domainPost.cid);
            expect(aliasRow).to.exist;
        ...
        it("Spec: same signer maps to a different anonymized author address across different posts", async () => {
            ...
            const firstAlias = context.subplebbit._dbHandler.queryPseudonymityAliasForPost(authorSigner.publicKey, firstPost.cid);
            const secondAlias = context.subplebbit._dbHandler.queryPseudonymityAliasForPost(authorSigner.publicKey, secondPost.cid);
            expect(firstAlias).to.exist;
            expect(secondAlias).to.exist;
            expect(firstAlias.aliasPrivateKey).to.not.equal(secondAlias.aliasPrivateKey);
        ...
        it("Spec: anonymized publication keeps author displayName while stripping wallets/avatar/flair fields", async () => {
            ...
            const aliasRow = context.subplebbit._dbHandler.queryPseudonymityAliasForPost(authorSigner.publicKey, noisyPost.cid);
            const aliasSigner = await context.publisherPlebbit.createSigner({ privateKey: aliasRow.aliasPrivateKey, type: "ed25519" });
        ...
        it("Spec: anonymized publication omits author.previousCommentCid", async () => {
            ...
            const aliasRow = context.subplebbit._dbHandler.queryPseudonymityAliasForPost(chainAuthor.publicKey, chainedPost.cid);
            const aliasSigner = await context.publisherPlebbit.createSigner({ privateKey: aliasRow.aliasPrivateKey, type: "ed25519" });
        ...
        it("Spec: anonymized publication preserves original author fields in comment.original while public fields are stripped except displayName", async () => {
            ...
            const aliasRow = context.subplebbit._dbHandler.queryPseudonymityAliasForPost(authorSigner.publicKey, authoredPost.cid);
            expect(aliasRow).to.exist;
        ...
        it("Spec: comment edit signed by original author is accepted and re-signed with anonymized author key", async () => {
            ...
            const aliasRow = context.subplebbit._dbHandler.queryPseudonymityAliasForPost(editSigner.publicKey, editablePost.cid);
            expect(aliasRow).to.exist;
            const aliasSigner = await context.publisherPlebbit.createSigner({ privateKey: aliasRow.aliasPrivateKey, type: "ed25519" });
        ...
        it("Spec: purging an anonymized post without postCid still removes its alias mapping", async () => {
            ...
            const aliasAfterPurge = context.subplebbit._dbHandler.queryPseudonymityAliasByCommentCid(post.cid);
        ...
        it("Spec: purging an anonymized comment removes only that thread's alias mapping", async () => {
            ...
            const aliasFirst = context.subplebbit._dbHandler.queryPseudonymityAliasByCommentCid(firstPost.cid);
            const aliasSecond = context.subplebbit._dbHandler.queryPseudonymityAliasByCommentCid(secondPost.cid);
            expect(aliasFirst).to.exist;
            expect(aliasSecond).to.exist;

            await context.subplebbit._dbHandler.purgeComment(firstPost.cid);

            const aliasAfterFirstPurge = context.subplebbit._dbHandler.queryPseudonymityAliasByCommentCid(firstPost.cid);
            expect(aliasAfterFirstPurge).to.be.undefined;
            const aliasAfterSecond = context.subplebbit._dbHandler.queryPseudonymityAliasByCommentCid(secondPost.cid);
        ...
        it("Spec: sub owner can resolve an anonymized author address back to the original author address", async () => {
            ...
            const aliasRow = context.subplebbit._dbHandler.queryPseudonymityAliasForPost(authorSigner.publicKey, resolvablePost.cid);
            expect(aliasRow).to.exist;
            expect(aliasRow?.originalAuthorSignerPublicKey).to.equal(authorSigner.publicKey);
        ...
        it("Spec: disabling per-post anonymity stops anonymization for new comments without rewriting old ones", async () => {
            ...
            try {
                ...
                const alias = localContext.subplebbit._dbHandler.queryPseudonymityAliasByCommentCid(plainPost.cid);
        ...
        it("Spec: banning an anonymized comment maps to original and alias author addresses in per-post mode", async () => {
            ...
            try {
                ...
                const aliasRow = localContext.subplebbit._dbHandler.queryPseudonymityAliasForPost(localAuthor.publicKey, post.cid);
                expect(aliasRow).to.exist;
    ...
    describe("remote loading with anonymized comments", () => {
        describe("preloaded pages", () => {
            ...
            before(async () => {
                ...
                const aliasRow = sharedContext.subplebbit._dbHandler.queryPseudonymityAliasForPost(
        ...
        describe.sequential("paginated pages", () => {
            ...
            before(async () => {
                ...
                const aliasRow = paginatedContext.subplebbit._dbHandler.queryPseudonymityAliasForPost(
                    paginatedSigningAuthor.publicKey,
                    paginatedContext.post.cid
                );
                const secondAliasRow = paginatedContext.subplebbit._dbHandler.queryPseudonymityAliasForPost(
                    paginatedSigningAuthor.publicKey,
