import {
    ...
    topScore,
    unsubscribeAllPubsubTopics, waitTillPublicationsArePublished
} from "../src/Util.js";
import {
    generateMockComment,
    generateMockPostWithRandomTimestamp,
    generateMockVote, getLatestSubplebbitAddress,
...
const testSorting = async (sort, parentComment) => {
    return new Promise(async (resolve, reject) => {
        ...
        let comments = [];
        for (let i = 0; i < 1; i++)
            comments.push(parentComment ? await generateMockComment(parentComment, clientPlebbit) : await generateMockPostWithRandomTimestamp(subplebbitAddress, clientPlebbit));
        ...
        assert(comments.every(comment => Boolean(comment.cid)));
        let votes = [];
