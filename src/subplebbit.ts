...
import { POSTS_SORT_TYPES, REPLIES_SORT_TYPES, SortHandler } from "./sort-handler";
...
export class Subplebbit extends EventEmitter {
    ...
    async initSignerIfNeeded() {
        if (this.dbHandler) {
            ...
            if (!dbSigner) {
                ...
                debug(
                    `Subplebbit has no signer in DB, will insert provided signer from createSubplebbitOptions into DB`
                );
                // @ts-ignore
    ...
    async prePublish(newSubplebbitOptions = {}) {
        ...
        const subplebbitIpfsNodeKey = (await this.plebbit.ipfsClient.key.list()).filter(
            (key) => key.name === this.address
        )[0];
        if (!subplebbitIpfsNodeKey) {
            ...
            debug(
                `Subplebbit key is already in ipfs node, no need to import (${JSON.stringify(subplebbitIpfsNodeKey)})`
            );
    ...
    async upsertAndPublishChallenge(challenge, trx) {
        try {
            ...
            await this.plebbit.ipfsClient.pubsub.publish(
                this.pubsubTopic,
    ...
    async processCaptchaPubsub(pubsubMsg) {
        ...
        if (msgParsed.type === PUBSUB_MESSAGE_TYPES.CHALLENGEREQUEST) await this.handleChallengeRequest(msgParsed);
            ...
            msgParsed.type === PUBSUB_MESSAGE_TYPES.CHALLENGEANSWER &&
        ...
        const syncComment = async (dbComment) => {
            ...
            if (!currentIpns || !shallowEqual(currentIpns, dbComment.toJSONCommentUpdate(), ["replies"])) {
                try {
                    ...
                    const [sortedReplies, sortedRepliesCids] = await this.sortHandler.generatePagesUnderComment(
                        dbComment
                    );
    ...
    async _addPublicationToDb(publication) {
        const trx: any = publication.vote ? undefined : await this.dbHandler.createTransaction(); // No need for votes to reserve a transaction
        try {
