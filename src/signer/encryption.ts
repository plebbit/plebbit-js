...
export const encryptEd25519AesGcm = async (plaintext, privateKeyBase64, publicKeyBase64) => {
  ...
  if (publicKeyBuffer.length !== 32) throw Error(`encryptEd25519AesGcm publicKeyBase64 '${publicKeyBase64}' ed25519 public key length not 32 bytes (${publicKeyBuffer.length} bytes)`)
...
export const decryptEd25519AesGcm = async (encrypted: Encrypted, privateKeyBase64, publicKeyBase64) => {
  ...
  const publicKeyBuffer = uint8ArrayFromString(publicKeyBase64, 'base64')
  if (publicKeyBuffer.length !== 32) throw Error(`decryptEd25519AesGcm publicKeyBase64 '${publicKeyBase64}' ed25519 public key length not 32 bytes (${publicKeyBuffer.length} bytes)`)
