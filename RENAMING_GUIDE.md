...
- [ ] `publication.subplebbitAddress` → replace with wire fields `communityPublicKey` + `communityName`; `communityAddress` is instance-only (computed as `communityName || communityPublicKey`)
