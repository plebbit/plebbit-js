...
The profile is designed around two distinct abuse patterns on 5chan-style boards:

1. Thread creation spam.
2. In-thread reply flood.

It uses different controls for each path so moderation load stays bounded while normal replying stays usable.

### Challenge 0 (`captcha-canvas-v3`, post-only, `pendingApproval: true`) protects scarce thread slots
...
### Challenge 1 (`captcha-canvas-v3`, reply-only, `pendingApproval: true`) also gates untrusted replies
...
- Votes are not used on 5chan boards, so no vote-specific challenge rules are needed.
